...
def calculo_conciliacion(fecha_inicio, fecha_fin):
    ...
    raw_partidas = list(
        ...
        ).values("docnr", "partida__blart", "ractt", "wsl", "drcrk", "rwcur","lifnr", "kunnr","koart","augbl","zuonr","budat", "partida__bktxt")
    )
    ...
    cuentas_contra_unicas = {
        p["ractt"] for p in raw_partidas if p.get("ractt") not in hkonts_ctas_real
    }

    # 2. Consultar el historial EXCLUSIVAMENTE dentro de documentos bancarios (ZR/ZH)
    conteo_historico = defaultdict(lambda: {
        "S_count": 0, "H_count": 0, 
        "S_monto": Decimal('0'), "H_monto": Decimal('0')
    })
    
    if cuentas_contra_unicas:
        estadisticas = PartidaPosicion.objects.filter(
            ractt__in=cuentas_contra_unicas, 
            drcrk__in=["S", "H"],
            partida__blart__in=TIPOS_DOCUMENTO_BANCARIO
        ).values("ractt", "drcrk").annotate(
        ...
        for est in estadisticas:
            ...
            conteo_historico[cta][f"{drcrk}_monto"] = abs(est["total_monto"] or Decimal('0'))
    ...
    for docnr, lineas in partidas_por_doc.items():
        ...
        for partida in lineas_banco:
            ...
            registro = DashboardConsolidado(
                ...
                cuenta_real=ractt,                          # Tu cambio
                cuenta_transitoria=cta_contra_principal,    # Tu cambio seguro
                cuenta_gasto="",  
